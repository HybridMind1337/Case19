...
require_once '../includes/pagination.php'; // Важни функции на системата
...
$mysql_check = mysqli_query($link, "SELECT * FROM contacts");
...
if (mysqli_num_rows($mysql_check) > 0) {
    ...
    $values4_acp    = array(
        'pagination_pm' => $pagination_out
    );
} else {
    $values3_acp   = array(
        'no_have_pms' => "<div class='alert alert-danger'>Няма съобщения!</div>"
    );
    $values4_acp[] = "";
}
