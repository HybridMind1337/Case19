...
$mysql_check = mysqli_query($link, "SELECT * FROM gallery");
...
if (mysqli_num_rows($mysql_check) > 0) {
    ...
    $values4_acp    = array(
        'pagination_pics' => $pagination_out
    );
} else {
    $values3_acp   = array(
        'no_have_pics' => "<div class='alert alert-danger'>Няма изображения!</div>"
    );
    $values4_acp[] = "";
}
...
if (strpos($_SERVER["REQUEST_URI"], '/edit_gallery.php') !== false) {
    $is_gallery_page_acp = array(
        'is_gallery_page_acp' => 1
    );
}
//end
