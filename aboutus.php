...
$page_title = array(
    'page_title' => $lang_sys['lang_aboutus']
); // Заглавие на страницата

$get = mysqli_query($link, "SELECT * FROM aboutus") or die(mysqli_error($link));
$row = mysqli_fetch_assoc($get);
@mysqli_free_result($get);
$aboutus2 = htmlspecialchars_decode($row['aboutus']);
$aboutus  = array(
    'aboutus' => $aboutus2
);

$tpl = $mustache->loadTemplate('aboutus'); 
echo $tpl->render($page_title + $values + $values4 + $lang_sys + $banner88x31 + $banner468x60 + $aboutus + $poll_print + $poll_send_vote); 